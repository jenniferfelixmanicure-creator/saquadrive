...
router.get("/rides/history", authenticate, async (req: AuthRequest, res) => {
  try {
    const rows = await db.select({
      id: ridesTable.id, originAddress: ridesTable.originAddress, originLat: ridesTable.originLat,
      originLng: ridesTable.originLng, destAddress: ridesTable.destAddress, destLat: ridesTable.destLat,
      destLng: ridesTable.destLng, rideType: ridesTable.rideType, price: ridesTable.price,
      status: ridesTable.status, distance: ridesTable.distance, duration: ridesTable.duration,
      waitTimeFee: ridesTable.waitTimeFee, promoCode: ridesTable.promoCode, promoDiscount: ridesTable.promoDiscount,
      createdAt: ridesTable.createdAt, completedAt: ridesTable.completedAt,
      driverName: usersTable.name, driverRating: usersTable.driverRating,
      vehicleModel: usersTable.vehicleModel, vehiclePlate: usersTable.vehiclePlate, vehicleColor: usersTable.vehicleColor,
    }).from(ridesTable)
    ...
    res.json(rows.map(r => ({
      id: r.id, originAddress: r.originAddress, originLat: r.originLat, originLng: r.originLng,
      destAddress: r.destAddress, destLat: r.destLat, destLng: r.destLng, rideType: r.rideType,
      price: r.price, status: r.status, distance: r.distance, duration: r.duration,
      waitTimeFee: r.waitTimeFee, promoCode: r.promoCode, promoDiscount: r.promoDiscount,
      createdAt: r.createdAt, completedAt: r.completedAt,
      driver: r.driverName ? { name: r.driverName, rating: r.driverRating, vehicleModel: r.vehicleModel, vehiclePlate: r.vehiclePlate, vehicleColor: r.vehicleColor ?? null } : null,
    })));
...
router.get("/rides/driver/stats", authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const [totals] = await db.select({ totalRides: sql<number>`count(*)::int`, totalEarnings: sql<number>`coalesce(sum(${ridesTable.price}), 0)` })
    ...
    const [weekly] = await db.select({ weekRides: sql<number>`count(*)::int`, weekEarnings: sql<number>`coalesce(sum(${ridesTable.price}), 0)` })
    ...
    const [monthly] = await db.select({ monthRides: sql<number>`count(*)::int`, monthEarnings: sql<number>`coalesce(sum(${ridesTable.price}), 0)` })
    ...
    const [today] = await db.select({ todayRides: sql<number>`count(*)::int` })
    ...
    res.json({ totalRides: totals?.totalRides ?? 0, totalEarnings: totals?.totalEarnings ?? 0, weekRides: weekly?.weekRides ?? 0, weekEarnings: weekly?.weekEarnings ?? 0, monthRides: monthly?.monthRides ?? 0, monthEarnings: monthly?.monthEarnings ?? 0, todayRides: today?.todayRides ?? 0, rating: driver?.driverRating ?? 5.0 });
  } catch (err) { req.log.error(err); res.status(500).json({ message: "Erro interno" }); }
});
