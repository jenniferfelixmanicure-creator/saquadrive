...
function base64UrlDecode(str: string): string {
  ...
  try { if (typeof atob === "function") return atob(padded); } catch {}
  const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
  let output = ""; let i = 0;
  while (i < padded.length) {
    const e1 = chars.indexOf(padded[i++]); const e2 = chars.indexOf(padded[i++]);
    const e3 = chars.indexOf(padded[i++]); const e4 = chars.indexOf(padded[i++]);
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    loadSession().finally(() => { clearTimeout(giveUp); setIsLoading(false); });
  }, []);
  ...
  async function apiFetch(path: string, options: RequestInit = {}): Promise<Response> {
    ...
    if (!isFormData) Object.assign(headers, options.headers as Record<string, string> ?? {});
    else if (options.headers) {
      const h = options.headers as Record<string, string>;
      for (const k of Object.keys(h)) { if (k.toLowerCase() !== "content-type") headers[k] = h[k]; }
    }
    ...
    if (res.status === 401 && refreshToken && !refreshingRef.current) {
      ...
      try {
        const refreshRes = await fetch(`${API_URL}/api/auth/refresh`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ refreshToken }) });
        if (refreshRes.ok) {
          const data = await refreshRes.json() as { token: string; refreshToken: string };
          setToken(data.token); setRefreshToken(data.refreshToken);
          await Promise.all([AsyncStorage.setItem(TOKEN_KEY, data.token), AsyncStorage.setItem(REFRESH_TOKEN_KEY, data.refreshToken)]);
  ...
  async function login(email: string, password: string) {
    ...
    const userData: User = { id: data.user.id, name: data.user.name, email: data.user.email, phone: data.user.phone, isApproved: data.user.isApproved, rgStatus: data.user.rgStatus as User["rgStatus"], passengerRating: 5.0, profilePhotoUrl: data.user.profilePhotoUrl };
    const saves = [AsyncStorage.setItem(USER_KEY, JSON.stringify(userData)), AsyncStorage.setItem(TOKEN_KEY, data.token)];
    if (data.refreshToken) { saves.push(AsyncStorage.setItem(REFRESH_TOKEN_KEY, data.refreshToken)); setRefreshToken(data.refreshToken); }
  ...
  async function updateUser(updates: Partial<User>) {
    ...
    if (token && (updates.name || updates.phone)) {
      try { await apiFetch("/api/users/me", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: updates.name, phone: updates.phone }) }); } catch {}
    }
  }
